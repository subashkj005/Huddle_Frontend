...
function AdminOTP() {
  ...
  const handleSubmit = () => {
    const finalOtp = joinOtp(otp);
    const data = { email: email, otp: finalOtp };

    showAdminLoading();

    axiosInstance
      .post(`${ADMIN_AUTH}/otp_confirm`, data)
      .then((res) => {
        console.log(res, 'success res')
        if (res.status === 201) {
          toast.success(res.data.message)
          const newState = { ...location.state, email: '' };
          navigate('/admin', { state: newState });
        }
      })
      .catch((err) => {
    ...
    <>
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap items-center h-screen">
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:block xl:w-1/2">
                ...
                <div className="flex space-x-4">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      value={digit}
                      maxLength="1"
                      className="w-10 h-10 border border-gray-300 rounded text-center focus:outline-none focus:border-blue-500 focus:ring focus:ring-blue-200"
